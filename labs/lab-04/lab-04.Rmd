...
Between 2019 and 2024, one of the most significant global events was the COVID-19 pandemic, which profoundly affected housing and rental markets. In this analysis, we focus on apartment buildings, aiming to examine how their rental prices evolved during this period by visualizing temporal dynamics in the data.

First, we filter the rental dataset to apartment buildings only and add three useful variables:
...
Let's produce the first plot about monthly rent trends for apartment buildings, separated by bedroom type and apartment quality. To determine the aesthetics (channels/encoding variables), we map time (month) to the x-axis, average rent (mean_rent) to the y-axis. There are two additional dimensions in the data: apartment quality (quality_group), which is represented by color, and bedroom type (bed_group), which is shown through faceting. 
